...
const superpingText = `Hello! I've invited you to this private chatroom to have a quick chat. Please click the "join this room" button to join.`;
const cmSuperpingText = `Hello! I'm a Community Manager at Stack Overflow and I've invited you to a private chatroom to have a quick chat. Please click the "join this room" button to join.`;
...
(function init() {
  ...
  if (location.pathname.includes('/users/') && $('#tabs a').first().hasClass('youarehere')) {
    ...
    if(window.location.host === "chat.stackexchange.com") {
        // Clone search form and 'convert' it into create new private room form with userId set in description so we can retrieve it to grant access afterwards
        const sFormCM = $('.usercard-xxl form[action="/search"]');
        const pFormCM = sFormCM.clone(true, true).attr({
            'action': '/rooms/save',
            'method': 'post'
        });
        ...
        // Insert necessary fields into private form
        // The secret's in the "defaultAccess" and "noDupeCheck" params
        $('#fkey').clone().prependTo(pFormCM);
        pFormCM.find('.button').addClass('private-button').val('(CM) create A51 private room with user');
        pFormCM.find('input[name="user"]').attr('name', 'description').val((i, v) => 'grant-write:' + v);
        pFormCM.find('input[name="q"]').attr('name', 'name').val(`Room for ${curruserName} and ${username}`);
        pFormCM.append(`
      <input type="hidden" name="defaultAccess" value="request" />
      <input type="hidden" name="host" value="area51.meta.stackexchange.com" />
      <input type="hidden" name="tags" value="" />
      <input type="hidden" name="noDupeCheck" value="true" />`);

        // Insert this private room form before the search form
        pFormCM.insertBefore(sFormCM);
    }
    return;
  }
  ...
  // Room created with userId in room description
    ...
    // Add spinner
    addFullPageBlocker("Granting write access, please wait...");
  ...
  // If in chat room with first two messages containing userId, show superping button
      ...
      let findShowSuperping = function () {
          ...
          const inlineButtons = $(`<span class="inline-buttons">&nbsp;<a href="${location.origin}/users/${userId}" target="_blank" class="button" title="view chat profile">profile</a><a class="button superpinger" title="mod superping user">superping (normal)</a><a class="button superpingstaff" title="mod superpingstaff">superping (as CM)</a></span>`);
        ...
        inlineButtons.on('click', '.superpingstaff', function () {
            $(this).remove();
            $.post(`/chats/${CHAT.CURRENT_ROOM_ID}/messages/new`, {
                'fkey': fkey,
                'text': `@@${userId} ${cmSuperpingText}`
            });
            return false;
        }).appendTo(msgs);
    }
